...
class ReinsDinoVisionTransformer(DinoVisionTransformer):
    ...
    def train(self, mode: bool = True):
        ...
        if self.in_chans !=3:
            set_requires_grad(self, ["reins", "linear", "patch_embed"])
            set_train(self, ["reins", "linear", "patch_embed"])
...
class ReinsDinoVisionTransformer_3_head(DinoVisionTransformer):
    ...
    def forward_features_DAT(self, x, masks=None):
        B, _, h, w = x.shape
        x = self.prepare_tokens_with_masks(x, masks)

        for idx, blk in enumerate(self.blocks):
            x = blk(x)
            out1 = self.reins1.forward(x, idx, batch_first=True, has_cls_token=True)
            out2 = self.reins2.forward(x, idx, batch_first=True, has_cls_token=True)  # frozen
            x = (out1 + out2) / 2  # 혹은 weighted sum
        return x
